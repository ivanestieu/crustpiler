...
pub fn lex_string_lit(lex: &mut logos::Lexer<Token>) -> Result<StringLit, LexError> {
    ...
    Ok(match chars.next() {
        Some('L') => StringLit {
            prefix: StringPrefix::Wide,
            value: interpret_string(&s[2..])?,
        },
        Some('u') => match chars
            .next()
            .ok_or(LexError::new(LexErrorKind::BadStringLiteral {
                text: String::from("'u' prefix should be followed by either '8' or a '\"'"),
            }))? {
            '8' => StringLit {
                prefix: StringPrefix::Utf8,
                value: interpret_string(&s[3..])?,
            },
            _ => StringLit {
                prefix: StringPrefix::Utf16,
                value: interpret_string(&s[2..])?,
            },
        },
        Some('U') => StringLit {
            prefix: StringPrefix::Utf32,
            value: interpret_string(&s[2..])?,
        },
        _ => StringLit {
            prefix: StringPrefix::None,
            value: interpret_string(&s[1..])?,
        },
    })
}
